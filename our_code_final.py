...
def neighbor_competition_loss(
    ...
    hardest_img_neg = img_neg_scores.max(dim=1).values
    hardest_txt_neg = txt_neg_scores.max(dim=1).values
    margin_t = torch.tensor(float(margin), device=y_img.device, dtype=y_img.dtype)
    loss_i2t = F.softplus(hardest_img_neg - pos_scores + margin_t).mean()
    loss_t2i = F.softplus(hardest_txt_neg - pos_scores + margin_t).mean()
...
def train_lora_postprocess(img_x: torch.Tensor,
    ...
    def _sample_txt_for(img_indices: torch.Tensor,
        ...
        if pool_text is None or pool_indices is None:
            ...
            ref_pool = fallback
        elif getattr(args, "caption_agg", "random") == "mean":
            rows = []
    ...
    def _forward(img_feats: torch.Tensor,
        ...
        if getattr(args, "lambda_neighbor_compete", 0.0) > 0:
            l_nbr = neighbor_competition_loss(
                y_img,
                y_txt,
                k=int(getattr(args, "neighbor_compete_k", 10)),
        ...
        total = (
            ...
            getattr(args, "lambda_neighbor_compete", 0.0) * l_nbr
        )
            ...
            if val_idx is not None or val_img_x is not None:
                with torch.no_grad():
                    if val_img_x is not None:
                        ...
                        align_txt_feats = _sample_txt_for(align_idx, val_cap_text, val_cap_indices, txt_x, apply_noise=False)
                        val_total, _, _, _, _, _, _, _, _ = _forward(img_val, txt_val, None, align_img_feats, align_txt_feats)
                        v = scalar_to_float(val_total)
                    else:
                        img_val = img_x[val_idx]
                        txt_val = _sample_txt_for(val_idx, cap_text, cap_indices, txt_x, apply_noise=False)
                        val_total, _, _, _, _, _, _, _, _ = _forward(img_val, txt_val)
                        v = scalar_to_float(val_total)
...
def maybe_postprocess(
    ...
    method_info = {
        ...
        "dimension_offset_ds": float(getattr(args, "dimension_offset_ds", getattr(args, "dimension_offset_dw", 0.0) or 0.0)),
